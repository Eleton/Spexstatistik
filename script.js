...
function intiAmazeChart() {
  ...
  var svg = svg0.append("svg").attr("width", width).attr("height", height);

  var verticaleLines = svg
    .selectAll("rect")
    .data(listOfGroups)
    .enter()
    .append("rect")
      ...
      circle
        ...
        .attr("cx", function (spexare) {
          ...
          if (g !== "Inte med") {
            ...
            var remainingGroups = spexare.groups.slice(
              yearIndex,
              spexare.groups.length
            );
            if (
              remainingGroups.every(function (g) {
                return g == "Inte med";
              })
            ) {
              return width;
        ...
        .attr("cy", function (spexare) {
          ...
          if (g !== "Inte med") {
            ...
            return (
              (Math.random() * height) / 16 + (height / 12) * experience + 10
            );
          } else {
            return 5;
          }
...
function initLineChart() {
  ...
  var x = d3.scale.linear().range([0, (2 * width) / 3]);
  ...
  x.domain(
    d3.extent([0, 1, 2, 3, 4, 5], function (d, i) {
      return i;
    })
  );
  y.domain(
    d3.extent([0, 0.65], function (d) {
      return d;
    })
  );
  ...
  var color = d3.scale.sqrt().domain([0, 10]).range([0, 360]);
  ...
  for (var i = 5; i < listOfYears.length; i++) {
    ...
    var lines = svg
      ...
      .style("stroke", function (d) {
        return d3.hsl(color(i), 1, 0.3).toString();
      });
    ...
    legend
      .on("mouseover", function () {
        ...
        d3.select("#line" + listOfYears.indexOf(d3.select(this).text())).style(
          "stroke-opacity",
          1
        );
      })
    ...
    lines
      .on("mouseover", function () {
        ...
        d3.select("#legend" + d3.select(this).attr("id").substring(4, 6)).attr(
          "fill-opacity",
          1
        );
      })
...
function initHeatMap() {
  ...
  var color = d3.scale.sqrt().domain([0, 20]).range([0, 60]);
  ...
  var heatChart = svg
    ...
    .attr("transform", function (d, i) {
      var x =
        3 +
        (height / listOfGroups.length) *
          (i - listOfGroups.length * Math.floor(i / listOfGroups.length));
      var y =
        2 +
        (height / listOfGroups.length) * Math.floor(i / listOfGroups.length);
      return "translate(" + x + "," + y + ")";
    });

  var heatRects = heatChart
    .append("rect")
    ...
    .attr("fill", function (d) {
      return d3.hsl(color(d), 1, 0.3).toString();
    })
  ...
  heatRects.on("click", function (d, i) {
    ...
    infoTextGroup
      .append("text")
      .text(
        listOfGroups[Math.floor(i / listOfGroups.length)] +
          " -> " +
          listOfGroups[i % listOfGroups.length]
      )
  ...
  var buttonData = [
    { text: "År till år", func: heatMap },
    { text: "Det förflutna", func: heatMap2 },
    { text: "Framtiden", func: heatMap3 },
  ];

  var buttonGroups = infoWindow
    .selectAll("g")
    .data(buttonData)
    .enter()
    .append("g")
    .attr("transform", function (d, i) {
      return "translate(" + (i * width) / 6 + "," + (height - 70) + ")";
    });

  var buttonRects = buttonGroups
    .append("rect")
    .attr("fill", "#ddd")
    .attr("opacity", 0.3)
    .attr("width", width / 6 - 20)
    .attr("height", 50)
    .attr("fill", "black");
  ...
  function changeGraph(callback) {
    ...
    heatRects //.attr("opacity", 1)
      ...
      .style("fill-opacity", function (d, i) {
        return flattenedHeatMap[i] === 0 ? 0.2 : 1;
      })
      .attr("fill", function (d, i) {
        return d3.hsl(color(flattenedHeatMap[i]), 1, 0.3).toString();
      });
  }
}
...
function findIndex(entry) {
  ...
  for (var i = 0; i < spexarna.length; i++) {
    if (
      spexarna[i].firstname + spexarna[i].lastname ==
...
function heatMap() {
  var heatMatrix = listOfGroups.map(function (d) {
    return new Array(listOfGroups.length + 1)
      .join("0")
      .split("")
      .map(parseFloat);
  });
